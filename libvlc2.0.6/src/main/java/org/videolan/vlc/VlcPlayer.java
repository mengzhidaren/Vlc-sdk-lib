...
public class VlcPlayer implements MediaPlayerControl, Handler.Callback, IVLCVout.Callback {
    ...
    private static MediaPlayer getMediaPlayer(LibVLC libVLC) {
        if (isInstance) {
            if (staticMediaPlayer == null) {
                staticMediaPlayer = new MediaPlayer(libVLC);
            }
        } else {
    ...
    public void setMedia(Media media, LibVLC libVLC) {
        othereMedia = true;
        if (mMediaPlayer == null) mMediaPlayer = getMediaPlayer(libVLC);
    ...
    private void opendVideo() {
        ...
        if (mMediaPlayer == null) {
            mMediaPlayer = getMediaPlayer(libVLC);
        ...
        if (!mMediaPlayer.getVLCVout().areViewsAttached() && isAttached && surface != null) {
            ...
            mMediaPlayer.getVLCVout().setVideoSurface(new Surface(surface), null);
        ...
        canPause = false;
        isPlayError = false;
        LogUtils.i(tag, "isAttached=" + isAttached + " isInitStart=" + isInitStart + " " + isAttachedSurface);
